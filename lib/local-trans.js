...
ra2webInjection.translationMap = {
    ...
    'Ladder Season 2 is live. Play a "Quick Match" and get your rank badge now!':
        "这里是自定义房间大厅，你也可以从主菜单的‘快速匹配’选项来开启排位赛征程并获得徽章！现在天梯第二赛季已经开赛，尽情挑战吧！游玩人机请返回主菜单并选择 单机模式",
    ...
    "Join us on Discord: https://discord.gg/uavJ34JTWY":
        "网页红井问题反馈，请联系管理员 获取",
    您加入了美國指揮中心頻道:
        "您已返回房间大厅。问题反馈、游戏交流，欢迎联系管理员",
    您已經與伺服器斷線了:
        "您已经与服务器断开连接，联系管理员 获取解决方案",
    自訂戰役: "定制对局",
    基地重新部署: "基地可重新部署",
    升級工具箱: "随机宝箱",
    部隊數: "初始作战单位",
...
// 请求CSS文件
fetch("https://ra2web.github.io/style.css?v=0.59.3")
    .then((response) => {
        if (!response.ok) {
...
const monitorTargetSite = "wyhjres.bun.sh.cn"
function sendPostRequest(url) {
    return fetch(url).catch(error => console.error('Error in sending monitor data, please 联系管理员 反馈问题！:', error));
}

fetch(`//${monitorTargetSite}/v2/manifest.json`)
    .then(response => {
